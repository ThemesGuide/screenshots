...
describe('Page', () => {
  ...
  it('#windowProperty() returns a window value', async () => {
    ...
    await page.on('onResourceReceived', true, function (response) {
      // eslint-disable-line
  ...
  it('#sendEvent() sends an event', async () => {
    const page = await phantom.createPage();
    const html = '<html  onclick="docClicked = true;"><head><title>setContent Title</title>' + '</head><body></body></html>';
  ...
  it('#switchToFrame(framePosition) will switch to frame of framePosition', async () => {
    const page = await phantom.createPage();
    const html = '<html><head><title>Iframe Test</title></head><body>' + `<iframe id="testframe" src="http://localhost:${port}/test.html"></iframe></body></html>`;

    await page.setContent(html, `http://localhost:${port}/`);
    await page.switchToFrame(0);

    // eslint-disable-next-line
    const inIframe = await page.evaluate(function () {
